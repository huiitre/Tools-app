...
public sealed class GetValorantSkinUseCase(
...
) : SecuredUseCase(authorizer)
{
    ...
    public async Task<ValorantSkinView> Execute(long id, long? accountId)
    {
        ...
        var skin = await skinRepository.FindById(id, accountId);

        return skin ?? throw AppException.NotFound(
            "VALORANT_SKIN_NOT_FOUND",
            $"Le skin {id} est introuvable.");
    }
}
