...
{
    private List<VpnPeerDto> Peers { get; } =
    [
    ...
    {
        return Task.FromResult<IReadOnlyList<VpnPeerDto>>(Peers);
    }

    public Task<VpnPeerDto> AddPeerAsync(string name)
    {
        if (Peers.Any(peer => peer.Name == name))
        {
            throw AppException.Conflict("VPN_PEER_EXISTS", $"Le peer « {name} » existe déjà.");
        }

        // Comme next_free_ip : la plus haute adresse occupée + 1, les trous ne sont jamais recyclés.
        var octet = Peers.Select(peer => int.Parse(peer.Ip.Split('.')[3])).DefaultIfEmpty(1).Max() + 1;

        var peer = new VpnPeerDto(
            name,
            $"10.13.13.{octet}",
