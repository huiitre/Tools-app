...
public sealed class GetValorantSkinByLevelUseCase(
...
) : SecuredUseCase(authorizer)
{
    ...
    public async Task<ValorantSkinView> Execute(Guid levelAssetId, long? accountId)
    {
        ...
        var skin = await skinRepository.FindByLevelAssetId(levelAssetId, accountId);

        return skin ?? throw AppException.NotFound(
            "VALORANT_SKIN_NOT_FOUND",
            $"Aucun skin ne correspond au level {levelAssetId}.");
    }
}
