...
import TemtemContextFloating from './TemtemContextFloating.vue'

// Même infobulle que Dofus (ItemContextTrigger) et Palworld (PalContextTrigger) : mêmes
// décalages, mêmes marges de bord, même bascule au-dessus / à gauche quand la place manque.
const props = defineProps<{
  temtem?: TemtemSummary
  // La popup « ajouter à une équipe » se superpose à l'infobulle : on la coupe pendant ce temps.
  disabled?: boolean
}>()
...
const onEnter = async (e: MouseEvent) => {
  if (!props.temtem || props.disabled) return

  visible.value = true
  await nextTick()

  const el = document.querySelector('.temtem-context-floating') as HTMLElement | null
