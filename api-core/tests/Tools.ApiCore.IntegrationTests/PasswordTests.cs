...
public sealed class PasswordTests : IClassFixture<ApiCoreWebApplicationFactory>
{
    private readonly ApiCoreWebApplicationFactory factory;
    private readonly InMemoryAuthStore store;

    public PasswordTests(ApiCoreWebApplicationFactory factory)
    {
    ...
    [Fact]
    public async Task Set_password_is_refused_without_a_token()
    {
        var client = factory.CreateClient();
    ...
    [Fact]
    public async Task Set_password_is_refused_for_a_token_without_any_role()
    {
        store.AddUser(7, "user@example.com", withPasswordProvider: true);
        var client = factory.CreateClientForUser(7);
