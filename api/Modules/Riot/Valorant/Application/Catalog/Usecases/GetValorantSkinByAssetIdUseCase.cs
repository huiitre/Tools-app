...
public sealed class GetValorantSkinByAssetIdUseCase(
...
) : SecuredUseCase(authorizer)
{
    ...
    public async Task<ValorantSkinView> Execute(Guid assetId, long? accountId)
    {
        ...
        var skin = await skinRepository.FindByAssetId(assetId, accountId);

        return skin ?? throw AppException.NotFound(
            "VALORANT_SKIN_NOT_FOUND",
            $"Le skin {assetId} est introuvable.");
    }
}
