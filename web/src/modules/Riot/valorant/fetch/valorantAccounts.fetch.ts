...
export async function listAccounts(): Promise<ValorantAccount[]> {
  const { data } = await clientCore.get<ValorantAccount[]>('/riot/valorant/accounts')
  return data
}
...
export async function linkAccount(
  ...
  const { data } = await clientCore.post('/riot/valorant/accounts', { refreshToken, region, label })
  return data
}

export async function unlinkAccount(accountId: number): Promise<void> {
  await clientCore.delete(`/riot/valorant/accounts/${accountId}`)
}

export async function renameAccount(accountId: number, label: string): Promise<ValorantAccount> {
  const { data } = await clientCore.put<ValorantAccount>(`/riot/valorant/accounts/${accountId}`, { label })
  return data
}
